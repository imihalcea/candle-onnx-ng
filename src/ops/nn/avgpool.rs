...
pub(crate) struct AveragePool;

impl OnnxOp for AveragePool {
    fn eval(&self, node: &ComputeNode) -> Result<OpOutput, OnnxOpError> {
        ...
        if let Some(d) = dilations {
            if d.iter().any(|&v| v != 1) {
                let error = format!("AvgPool with dilation != 1, {dilations:?}");
                return Err(OnnxOpError::UnsupportedAttribute(error));
            }
        }
        if let Some(d) = pads {
            if d.iter().any(|&v| v != 0) {
                let error = format!("AvgPool with pads != 0, {pads:?}");
                return Err(OnnxOpError::UnsupportedAttribute(error));
            }
        }
        ...
        let (k1, k2) = match kernel_shape {
            ...
            _ => {
                let error = format!("only 2d AvgPool is supported, kernel shape {kernel_shape:?}");
                return Err(OnnxOpError::UnsupportedAttribute(error));
            }
        };
        ...
        let ys = match strides {
            ...
            Some(strides) => {
                let error = format!("only 2d AvgPool is supported, strides {strides:?}");
                return Err(OnnxOpError::UnsupportedAttribute(error));
            }
        };
